...
// RLSが有効なため service_role キーで投入（RLSバイパス）。
// キーは env (SUPA_SERVICE_KEY / SUPABASE_SERVICE_ROLE_KEY) か macOS Keychain から取得。
const SUPABASE_URL = "https://lzzwrklsgwuuahxvbabl.supabase.co";
const KEYCHAIN_SERVICE = "mandala-supabase-service-role";
...
const SERVICE_KEY =
  process.env.SUPA_SERVICE_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY || keyFromKeychain();
if (!SERVICE_KEY) {
  console.error("service_roleキーが見つかりません（env SUPA_SERVICE_KEY か Keychain）。");
  process.exit(1);
}
// 投入先アカウント（アプリのログインメール）。env で上書き可。
const TARGET_EMAIL = process.env.SUPA_EMAIL || "yuta.suzuki@wein.co.jp";
...
const TASKS = [
  ...
  { no: "1-1", label: "週2回の自主練を習慣化する（後半期継続）", deadline: "2026-09-30", type: "habit",
    kdi: { label: "自主練", freq: "weekly", target: 8, deadline: "2026-12-31" } },
  { no: "1-2", label: "週2回、音楽制作または理論学習の時間をつくる", deadline: "2026-09-30", type: "habit",
    kdi: { label: "制作・理論学習", freq: "weekly", target: 8, deadline: "2026-12-31" } },
  ...
  { no: "2-1", label: "毎日日記を書く", deadline: "2026-07-31", type: "habit",
    kdi: { label: "日記", freq: "daily", deadline: "2026-12-31" } },
  { no: "2-2", label: "毎日10分間の瞑想を行う", deadline: "2026-07-31", type: "habit",
    kdi: { label: "瞑想", freq: "daily", deadline: "2026-12-31" } },
  ...
  { no: "2-4", label: "月2本以上映画を観る", deadline: "2026-12-31", type: "habit",
    kdi: { label: "映画鑑賞", freq: "monthly", target: 2 } },
  { no: "2-5", label: "月1回以上お笑いのライブ・舞台を観に行く", deadline: "2026-12-31", type: "habit",
    kdi: { label: "お笑いライブ", freq: "monthly", target: 1 } },
  ...
  { no: "4-1", label: "仕事用X・プライベートXの両方を毎日投稿する", deadline: "2026-07-31", type: "habit",
    kdi: { label: "X投稿（仕事/プライベート）", freq: "daily", deadline: "2026-12-31" } },
  { no: "4-2", label: "仕事関連のNote記事を月2本投稿する（習慣化）", deadline: "2026-10-31", type: "habit",
    kdi: { label: "Note投稿", freq: "monthly", target: 2, deadline: "2026-12-31" } },
  ...
  { no: "5-7", label: "月1回以上、起業・事業構想の時間を確保する", deadline: "2026-12-31", type: "habit",
    kdi: { label: "起業・事業構想", freq: "monthly", target: 1 } },
  { no: "5-8", label: "競合・成功事例を毎週リサーチしメモする", deadline: "2026-08-31", type: "habit",
    kdi: { label: "競合・事例リサーチ", freq: "weekly", target: 4, deadline: "2026-12-31" } },

  // 6. 魅力的な人間になる
  { no: "6-1", label: "週4回ジムに通う", deadline: "2026-07-31", type: "habit",
    kdi: { label: "ジム", freq: "weekly", target: 16, deadline: "2026-12-31" } },
  { no: "6-2", label: "毎日体重を測定する", deadline: "2026-07-31", type: "habit",
    kdi: { label: "体重測定", freq: "daily", deadline: "2026-12-31" } },
  { no: "6-3", label: "毎日最低10分は体を動かす", deadline: "2026-07-31", type: "habit",
    kdi: { label: "運動10分", freq: "daily", deadline: "2026-12-31" } },
  ...
  { no: "7-8", label: "月1回以上、新しい友人・人脈を作る", deadline: "2026-12-31", type: "habit",
    kdi: { label: "新しい友人・人脈", freq: "monthly", target: 1 } },
  ...
  { no: "8-3", label: "家庭の出費（家賃・水道光熱費・食費）を月30万円以内に抑える", deadline: "2026-12-31", type: "habit",
    kdi: { label: "家庭出費30万以内", freq: "monthly", target: 1 } },
  ...
  { no: "8-5", label: "投資運用スタイルを確立し定期積立を継続する", deadline: "2026-12-31", type: "habit",
    kdi: { label: "投資積立", freq: "monthly", target: 1 } },
  ...
  { no: "8-8", label: "サブスクを月3万円以内に抑える", deadline: "2026-07-31", type: "habit",
    kdi: { label: "サブスク3万以内", freq: "monthly", target: 1 } },
];

// 既存の同名チャートを所有者問わず子レコードごと削除（重複/誤投入の掃除・冪等化）
async function cleanupByChartNames(names) {
  const { data: charts, error } = await supabase.from("charts").select("id, user_id").in("name", names);
  if (error) throw error;
  if (!charts || charts.length === 0) return;
  const chartIds = charts.map((c) => c.id);
  const { data: sgs } = await supabase.from("sub_goals").select("id").in("chart_id", chartIds);
  const sgIds = (sgs ?? []).map((s) => s.id);
  let taskIds = [];
  if (sgIds.length) {
    const { data: ts } = await supabase.from("tasks").select("id").in("sub_goal_id", sgIds);
    taskIds = (ts ?? []).map((t) => t.id);
  }
  let kdiIds = [];
  if (taskIds.length) {
    const { data: ks } = await supabase.from("kdis").select("id").in("task_id", taskIds);
    kdiIds = (ks ?? []).map((k) => k.id);
  }
  if (kdiIds.length) {
    await supabase.from("kdi_checks").delete().in("kdi_id", kdiIds);
    await supabase.from("kdis").delete().in("id", kdiIds);
  }
  if (taskIds.length) await supabase.from("tasks").delete().in("id", taskIds);
  if (sgIds.length) await supabase.from("sub_goals").delete().in("id", sgIds);
  await supabase.from("charts").delete().in("id", chartIds);
  console.log(`Cleaned ${chartIds.length} existing chart(s) [${names.join(", ")}] [sg=${sgIds.length} task=${taskIds.length} kdi=${kdiIds.length}]`);
}

async function main() {
  // 1. 対象ユーザーIDをメールから解決（admin API）
  ...
  if (!u) throw new Error(`auth user not found for ${TARGET_EMAIL}`);
  const userId = u.id;
  console.log(`Target user: ${TARGET_EMAIL} (${userId})`);
  await supabase.from("user_profiles").upsert({ id: userId });

  // 2. 既存チャートを一掃（新旧名・誤アカウント投入・重複の掃除）
  await cleanupByChartNames([CHART.name, "2026 プライベート"]);
  ...
  const kdiRows = TASKS.filter((t) => t.kdi).map((t) => {
    ...
    return {
      ...
      deadline: t.kdi.deadline ?? t.deadline,
    };
  });
  if (kdiRows.length) {
